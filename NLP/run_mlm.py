...
from properties_checker import compute_L2_norm, compute_difference, compute_linear_approx, compute_smoothness, compute_inner_product
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to download new datasets",
        required=False,
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to locally stored optimal parameters",
        required=False,
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.99,
        help="momentum constant for updating L",
    )
    parser.add_argument(
        "--beta_gap",
        type=float,
        default=0.99,
        help="momentum constant for updating gap",
    )
    parser.add_argument(
        "--check_opt",
        type=bool,
        default=False,
        help="check with stationary point",
    )
...
def main():
    ...
    wandb.init(project='new_transformer_project', config=args, name=args.name)
    ...
    if args.dataset_name is not None:
        ...
        raw_datasets = load_dataset('c4', 'en', data_dir = "/projectnb/aclab/datasets/c4/en/", streaming = True)
    ...
    #### Changed!
    ## If we want to use a pre-trained model, use the commented code, else use AutoModelForMaskedLM.from_config(config)
    ...
    if args.line_by_line:
        ...
        #### Changed! 
        ## Not all columns of data are needed for training, specify the remove_columns here depending on the dataset
    ...
    #### Changed!
    ## due to streaming mode, we don't have the len() function (since we access the dataset one batch at a time). We have to
    ## comment out everything with len() function here
    ...
    train_dataloader = LoadIt(None, root_dir = '/projectnb/aclab/tranhp/trainDataloader_c4/', max_workers = 1)
    # eval_dataloader = LoadIt(None, root_dir = '/projectnb/aclab/tranhp/evalDataloader_c4/', max_workers = 1)
    ...
    if args.opt == 'adam':
        ...
        optimizer = torch.optim.SGD(optimizer_grouped_parameters, lr=args.learning_rate, weight_decay = args.weight_decay, momentum = 0.9)
    ...
    if args.check_opt:
        opt = torch.load(args.path_opt)
        prev_grad = opt['current_grad_linear']
        prev_loss = opt['current_loss']
        prev_param = opt['current_param']
    else:
    ...
    for epoch in range(args.starting_epoch, args.num_train_epochs):
        ...
        if args.resume_from_checkpoint_torch is not None:
            ...
            current_grad_L = checkpoint['current_grad_L'] #gradient used to compute smoothness \nabla f(w_t,x_{t-1})
            current_grad_linear = checkpoint['current_grad_linear'] #gradient used to compute the current inner product \nabla f(w_t,x_{t})
            # current_param = checkpoint['current_param']
            # linear_actual = checkpoint['linear_actual']
            # current_convexity_gap = checkpoint['current_convexity_gap']
            # linear_approx_prev = checkpoint['linear_approx_prev']
            # num_actual = checkpoint['num_actual']
            # sum_dif = checkpoint['sum_dif']
            # prev_update = checkpoint['prev_update']
            # smoothness_ratio_prev = checkpoint['smoothness_ratio_prev']
            # smoothness_ratio_actual = checkpoint['smoothness_ratio_actual']
            dif = checkpoint['dif']
            if not args.check_opt:
                prev_loss = checkpoint['prev_loss']
                prev_grad= checkpoint['prev_grad']
                prev_param = checkpoint['prev_param']
        start = time.time()
        for i in range(resume_step+1, args.max_step):
            iteration+=1
            prev_batch = {key: value.to(device) if isinstance(value, torch.Tensor) else value for key, value in train_dataloader[i-1].items()}  #x_{t-1}
            batch = {key: value.to(device) if isinstance(value, torch.Tensor) else value for key, value in train_dataloader[i].items()} #x_t
            with accelerator.accumulate(model):
                ## first we want to get \nabla f(w_t,x_{t-1}) to compare with \nabla f(w_{t-1},x_{t-1})
    
                prev_batch_outputs = model(**prev_batch) #f(w_{t-1},x_{t-1})
                prev_batch_loss = prev_batch_outputs.loss
                current_loss = prev_batch_loss.detach().float()
                accelerator.backward(prev_batch_loss)
                i = 0
                with torch.no_grad():
                    for p in model.parameters():
                        ...
                        i+=1
                # now current_grad_L holds \nabla f(w_t,x_{t-1}) and current_param holds w_t
                # zero grad to do the actual update
                optimizer.zero_grad()
            
                # get the output at the current batch to update
                outputs = model(**batch) # f(w_t,x_t)
                loss = outputs.loss
                ...
                with torch.no_grad():
                    for p in model.parameters():
                        current_grad_linear[i].copy_(p.grad) # curent_grad_linear = \nabla f(w_t,x_{t})
                        i+=1
                # only goes in if we check the properties w.r.t stationary point
                if args.check_opt:
                    i = 0
                    with torch.no_grad():
                        for p in model.parameters():
                ...
                if (iteration%10000) != 1:
                    ...
                    denom+= current_loss - prev_loss
                    smoothness_ratio_actual = (-denom + num_actual)/(1/2*sum_dif)
                    smoothness_ratio_prev = (-denom + num)/(1/2*sum_dif)
                    current_convexity_gap = current_loss- prev_loss - linear_approx_prev #instantaneous convexity_gap
                    ...
                    inner_prod_grad = compute_inner_product(current_grad_linear, prev_grad)
                
               
                lr_scheduler.step()
                if (iteration % 10000 == 0 or (iteration ==1 and resume_step == 0)):
                    dic = { 'current_loss':current_loss, 'current_grad_linear': current_grad_linear, 'current_grad_L': current_grad_L, 'prev_loss': prev_loss,
                           ...
                           'total_loss': total_loss, 'prev_grad': prev_grad, 'prev_param': prev_param, 'linear_actual':linear_actual, 'linear_approx_prev': linear_approx_prev,
                           'num_actual': num_actual, 'current_convexity_gap': current_convexity_gap, 'sum_dif': sum_dif, 'prev_update': prev_update,'dif':dif,
                           'smoothness_ratio_prev': smoothness_ratio_prev, 'smoothness_ratio_actual': smoothness_ratio_actual}
                    if args.output_dir is not None:
                        output_dir = args.output_dir + str(epoch) + "_" + str(iteration)
                    #### Changed!
                    ## Save the dictionary
                    store_name = args.output_dir + str(epoch)+ "_" +str(iteration) + ".pth.tar"
                    ...
                    wandb.log(
                        {"train_loss": total_loss.item() / iteration,
                            ...
                            'linear_prev': linear_approx_prev,
            ...
            if completed_steps >= args.max_train_steps:
                break
        if not args.check_opt: 
            prev_batch = train_dataloader[iteration-1]
            prev_batch_outputs = model(**prev_batch)
            prev_batch_loss = prev_batch_outputs.loss
            current_loss = prev_batch_loss.detach().float()
            accelerator.backward(prev_batch_loss)
            i = 0
        ...
        dic = {'prev_loss': prev_loss, 'prev_grad': prev_grad, 'prev_param': prev_param, 'current_loss': current_loss, 'current_grad': current_grad
                           , 'current_param': current_param, 'smoothness': L, 'num': num , 'denom': denom, 'convexity_gap': convexity_gap, 'iteration': iteration,
                           'exp_avg_L_.99': exp_avg_L_1,'exp_avg_L_.9999': exp_avg_L_2,  "exp_avg_gap_.99":  exp_avg_gap_1, "exp_avg_gap_.9999":  exp_avg_gap_2,
                           'total_loss': total_loss, 'perplexity': perplexity, 'eval_loss':eval_loss}
        ####
